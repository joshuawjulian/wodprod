...
import { db } from '.';
import { movementPatternsTable, websiteRolesTable } from './schema';

async function main() {
	console.log('Seeding database...');
	await db.delete(movementPatternsTable);

	interface MovementPatternSeed {
		name: string;
		description: string;
	}

	const movementPatterns: MovementPatternSeed[] = [
		{
			name: 'Squat',
			description:
	...
	await db.insert(movementPatternsTable).values(movementPatterns);

	await db.delete(websiteRolesTable);
	await db.insert(websiteRolesTable).values([
		{
			name: 'admin'
		},
		{
